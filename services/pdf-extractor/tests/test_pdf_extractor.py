...
def test_extract_address_domestic_single_letter_province(pdf_domestic_address_malformed_no_province):
    """Un indirizzo domestico con provincia tronca ad 1 lettera o assente estrae comunque via, cap, comune e provincia=""."""
    addr = PdfExtractor(pdf_domestic_address_malformed_no_province).extract_address()
    assert addr.cap == "65015"
    assert addr.comune == "MONTESILVANO"
    assert addr.provincia == ""
    assert addr.stato_estero == ""
