...
Incidente reale: 3 PR consecutive (mergiate su questo repo **pubblico**) hanno introdotto fixture con nome+CF+email personale reali e nome+PIVA+PEC aziendali reali, copiati verbatim dal testo dumpato durante il debug di un PDF reale fornito dall'utente — scoperto solo a posteriori, non durante la review dei singoli PR. Ogni fixture che riproduce un bug trovato su un documento reale va scritta SEMPRE con dati fittizi (stesso pattern già in uso altrove nel file: `ROSSI MARIO`/`RSSMRA80A01H501U`, `ACME SRL`) — il filename del documento (`DOC_NNNNNN_NNNNN.pdf`) non è PII e può restare come riferimento in un commento, il contenuto testuale dumpato SEMPRE anonimizzato prima di incollarlo in una fixture. Un fix forward (nuovo commit che sanitizza) non rimuove il dato dalla history git già mergiata — se il repo è pubblico, valutare con l'utente se serve altro oltre al fix forward.
